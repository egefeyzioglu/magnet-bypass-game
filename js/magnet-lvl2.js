...
function updateInfo() {
    ...
    $('div#info div#text span#threshold').html((sensor.thresholdLow * (sensor.invert ? -1 : 1) * 1E6).toFixed(2) + " mT - " + (sensor.thresholdHigh * (sensor.invert ? -1 : 1) * 1E6).toFixed(2) + " mT");
...
function updateSensor() {
  ...
  sensor.triggered = (eval("bField(dipoles, sensor.location)." + sensor.alongAxis) * (sensor.invert ? -1 : 1) > sensor.thresholdLow) && (eval("bField(dipoles, sensor.location)." + sensor.alongAxis) * (sensor.invert ? -1 : 1) < sensor.thresholdHigh);
}
